...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    def _check_note_filters(self, note_detail: Dict) -> bool:
        """
        检查笔记是否符合过滤条件
        ...
        if config.DAYS > 0:
            ...
            if note_time:
                # 计算截止时间（毫秒）
                cutoff_time = int((datetime.now() - timedelta(days=config.DAYS)).timestamp() * 1000)
                
                if note_time < cutoff_time:
                    utils.logger.info(f"[XiaoHongShuCrawler._check_note_filters] Note {note_id} filtered: too old (time={note_time}, cutoff={cutoff_time})")
                    return False
        
        # 2. 检查话题词过滤
        if config.TOPIC_WORDS:
            title = note_detail.get('title', '')
            desc = note_detail.get('desc', '')
            content = f"{title} {desc}"
            
            # 标题或正文需包含至少一个话题词
            has_topic = any(word in content for word in config.TOPIC_WORDS)
            if not has_topic:
                utils.logger.info(f"[XiaoHongShuCrawler._check_note_filters] Note {note_id} filtered: no topic words found")
                return False
        ...
        # 检查是否满足最小值要求
        if config.MIN_LIKES > 0 and liked_count < config.MIN_LIKES:
            utils.logger.info(f"[XiaoHongShuCrawler._check_note_filters] Note {note_id} filtered: likes {liked_count} < {config.MIN_LIKES}")
            return False
        
        if config.MIN_FAVORITES > 0 and collected_count < config.MIN_FAVORITES:
            utils.logger.info(f"[XiaoHongShuCrawler._check_note_filters] Note {note_id} filtered: favorites {collected_count} < {config.MIN_FAVORITES}")
            return False
        
        if config.MIN_COMMENTS > 0 and comment_count < config.MIN_COMMENTS:
            utils.logger.info(f"[XiaoHongShuCrawler._check_note_filters] Note {note_id} filtered: comments {comment_count} < {config.MIN_COMMENTS}")
            return False
    ...
    async def start(self) -> None:
        async with async_playwright() as playwright:
            ...
            self.xhs_client = await self.create_xhs_client(None)  # No proxy support
    ...
    async def search(self) -> None:
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            crawled_notes_count = 0  # Track actual crawled notes count
            
            while crawled_notes_count < config.CRAWLER_MAX_NOTES_COUNT and page <= config.CRAWLER_MAX_PAGES_COUNT:
                if page < start_page:
                ...
                try:
                    utils.logger.info(f"[XiaoHongShuCrawler.search] search Xiaohongshu keyword: {keyword}, page: {page}, crawled: {crawled_notes_count}/{config.CRAWLER_MAX_NOTES_COUNT}")
                    note_ids: List[str] = []
                    ...
                    utils.logger.info(f"[XiaoHongShuCrawler.search] Fetching {len(filtered_items)} notes")
                    task_list = [
                    ...
                    for note_detail in note_details:
                        if note_detail:
                            ...
                            # 应用过滤条件
                            if not self._check_note_filters(note_detail):
                                utils.logger.info(f"[XiaoHongShuCrawler.search] Note {note_detail.get('note_id')} filtered out")
                                continue
                            
                            is_new = await xhs_store.update_xhs_note(note_detail)
                            await self.get_notice_media(note_detail)
                            note_ids.append(note_detail.get("note_id"))
                            xsec_tokens.append(note_detail.get("xsec_token"))
                            
                            # Only increment counter for new notes
                            if is_new:
                                crawled_notes_count += 1
                                utils.logger.info(f"[XiaoHongShuCrawler.search] Successfully crawled NEW note {note_detail.get('note_id')}, total: {crawled_notes_count}/{config.CRAWLER_MAX_NOTES_COUNT}")
                            else:
                                utils.logger.info(f"[XiaoHongShuCrawler.search] Updated existing note {note_detail.get('note_id')}, not counted towards limit")
                    
                    page += 1
                    utils.logger.info(f"[XiaoHongShuCrawler.search] Note details: {note_details}")
                    await self.batch_get_note_comments(note_ids, xsec_tokens)

                    # Check again after processing all notes in this page
                    if crawled_notes_count >= config.CRAWLER_MAX_NOTES_COUNT:
                        utils.logger.info(f"[XiaoHongShuCrawler.search] Completed crawling {crawled_notes_count} notes for keyword: {keyword}")
                        break
    ...
    async def create_xhs_client(self, httpx_proxy: Optional[str]) -> XiaoHongShuClient:
        """Create Xiaohongshu client (Ultra Minimal - No proxy support)"""
